...
Panel {
  ...
  // Every provider gets its own column, all visible at once. Panes ask for
  // paneWidth but stretch to split the viewport exactly when the fitted panel
  // comes out narrower (or wider) than the request, so nothing clips; below
  // paneMinWidth they hold firm and the panel pans horizontally instead.
  readonly property real paneWidth: Style.space(340)
  readonly property real paneMinWidth: Style.space(300)
  readonly property real paneSpacing: Style.space(28)

  function paneWidthFor(viewport) {
    var count = Math.max(1, providers.length)
    // The card eats one popup padding off the right of the viewport the
    // flickable reports, so leave it out before splitting.
    var fit = (viewport - Style.spacing.popupPadding - paneSpacing * (count - 1)) / count
    return Math.max(paneMinWidth, Math.floor(fit))
  }
  ...
  // last 10% of the funded credits lights the same alarm. With every provider
  // on screen, any one of them lights the bar icon.
  function balanceAlarmingFor(p) {
    var b = p ? (p.balance || null) : null
    return !!b && b.funded > 0 && b.remaining / b.funded <= 0.1
  }
  function limitAlarmingFor(p) {
    var w = bindingWindow(p)
    return !!w && w.percent >= 0.9
  }
  readonly property bool alarming: {
    for (var i = 0; i < providers.length; i++)
      if (limitAlarmingFor(providers[i]) || balanceAlarmingFor(providers[i])) return true
    return false
  }
  ...
  function dayTooltip(p, day, today) {
    if (!day) return ""
  ...
  function footerText() {
    ...
    var first = providers.length > 0 ? providers[0] : null
    if (first && first.syncEnabled && first.syncDeviceCount > 0)
      return "Merged from " + first.syncDeviceCount + " device" + (first.syncDeviceCount === 1 ? "" : "s")
    return ""
  }
  ...
  onOpenedChanged: if (opened) {
    nowMs = Date.now()
    if (panelFlick) { panelFlick.contentY = 0; panelFlick.contentX = 0 }
    usage.refreshLimits()
  ...
  IpcHandler {
    ...
    function next(): string { root.refreshNow(); return "ok" }
  }
  ...
  KeyboardPanel {
    ...
    contentWidth: panel.fittedContentWidth(
      Math.max(1, root.providers.length) * root.paneWidth
      + Math.max(0, root.providers.length - 1) * root.paneSpacing)
    ...
    PanelKeyCatcher {
      ...
      onMoveRequested: function(dx, dy) {
        if (dx !== 0)
          panelFlick.contentX = root.clamp(panelFlick.contentX + dx * Style.space(80), 0,
                                           Math.max(0, panelFlick.contentWidth - panelFlick.width))
        if (dy !== 0)
      ...
      Flickable {
        id: panelFlick
        anchors.fill: parent
        contentWidth: Math.max(width, column.implicitWidth)
        ...
        Column {
          id: column
          width: Math.max(panelFlick.width, implicitWidth)
          ...
          Row {
            ...
            Repeater {
              ...
              ProviderPane {
                ...
                p: modelData
                width: root.paneWidthFor(panelFlick.width)
              }
            }
          }
  ...
  // One provider's full dashboard: hero, status, balance and limits, tokens
  // by day, tokens by model. The panel shows one pane per enabled provider,
  // side by side, so nothing needs a click to reveal.
  component ProviderPane: Column {
    id: pane
    property var p: null

    readonly property var limits: root.limitWindows(p)
    readonly property var models: root.modelRows(p)
    readonly property var balance: p ? (p.balance || null) : null
    readonly property bool balanceAlarming: root.balanceAlarmingFor(p)
    ...
    PanelHero {
      ...
      visible: !!pane.p
      width: parent.width
      title: pane.p ? pane.p.providerName : ""
      meta: root.heroMeta(pane.p)
      ...
      iconComponent: Component {
        Item {
          id: heroMark
          property var candidates: root.iconCandidatesForProvider(pane.p, root.surface)
    ...
    // ---------- Status ----------
    BorderSurface {
      visible: !!pane.p && String(pane.p.usageStatusText || "") !== ""
      width: parent.width
      ...
      Text {
        ...
        text: pane.p ? String(pane.p.authHelpText || "") : ""
        color: root.dim
        font.family: root.fontFamily
        font.pixelSize: Style.font.caption
        wrapMode: Text.WordWrap
    ...
    Column {
      id: balanceSection
      visible: !!pane.balance
      width: parent.width
      spacing: Style.space(10)

      // The meter shows what is left, not what is used: a prepaid
      // account drains toward empty rather than filling toward a cap.
      readonly property real ratio: pane.balance && pane.balance.funded > 0
        ? root.clamp(pane.balance.remaining / pane.balance.funded, 0, 1)
        : -1
      ...
      Item {
        ...
        Text {
          id: balanceValue
          text: pane.balance ? root.formatMoney(pane.balance.remaining, pane.balance.currency) : ""
          color: pane.balanceAlarming ? root.urgent : root.foreground
          font.family: root.fontFamily
          font.pixelSize: Style.font.caption
          anchors.right: parent.right
          anchors.verticalCenter: parent.verticalCenter
      ...
      Meter {
        ...
        alarming: pane.balanceAlarming
      }

      Text {
        visible: text !== ""
        width: parent.width
        text: root.balanceDetailText(pane.balance)
        color: root.dim
        font.family: root.fontFamily
        font.pixelSize: Style.font.caption
      }
    ...
    Column {
      id: usageSection
      visible: !!pane.p && pane.p.recentDays && pane.p.recentDays.length > 0
      width: parent.width
      spacing: Style.spacing.md

      readonly property var days: pane.p ? (pane.p.recentDays || []) : []
      readonly property real peak: Math.max(1, root.weekPeak(pane.p))
    ...
    Column {
      ...
      Repeater {
        ...
        ModelRow {
          ...
          share: modelData.total / Math.max(1, pane.models[0].total)
        }
      }
    }
  }
  ...
  component DayRow: Item {
    ...
    PanelToolTip {
      ...
      text: root.dayTooltip(dayRow.p, dayRow.day, dayRow.today)
      fontFamily: root.fontFamily
    }
  }
